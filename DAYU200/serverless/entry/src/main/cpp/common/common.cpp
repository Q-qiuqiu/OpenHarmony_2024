...
bool GetMatFromRawFile(napi_env env, napi_value jsResMgr, const std::string &rawfileDir, const std::string &fileName,
                       Mat &srcImage) {
    ...
    OH_LOG_Print(LOG_APP, LOG_INFO, GLOBAL_RESMGR, TAG, "fileData size: %{public}d, srcImage：%{public}d",
...
bool cvtMat2Pixel(InputArray _src, OutputArray &_dst, int code) {
    OH_LOG_Print(LOG_APP, LOG_INFO, GLOBAL_RESMGR, TAG, "cvtMat2Pixel type %{public}d, code %{public}d",
                 _src.getMat_().type(), code);
    if (RGBA_8888 == code) {
...
bool WrapJsPixelInfoInfo(napi_env env, cv::Mat &outMat, napi_value &result) {
    ...
    napi_create_uint32(env, buffSize, &value);
    napi_set_named_property(env, result, "buffSize", value);

    value = nullptr;
    napi_create_uint32(env, outMat.cols, &value);
    napi_set_named_property(env, result, "cols", value);

    value = nullptr;
    napi_create_uint32(env, outMat.rows, &value);
