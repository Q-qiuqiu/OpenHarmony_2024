...
typedef enum {
    HI_VPC_CHN_PRIORITY = 0 // high channel priority
} hi_opt_attr_type;
...
typedef struct {
    hi_float x; // map1 源图片的点(对标OpenCV)
    hi_float y; // map2 源图片的点(对标OpenCV)
} hi_float_point;
...
typedef struct {
    hi_u32 interpolation;          // 0:bilinear 1:nearest
    hi_vpc_bord_type border_type;  // 0:寄存器固定值 1:边界复制模式
    hi_vpc_scalar scalar_value;    // 只支持3通道 scalar_value.value[0 1 2]
} hi_transform_config;
...
typedef enum {
    HI_MATRIX_2_PLUS_3 = 0, // 2X3 Matrix
    HI_MATRIX_3_PLUS_3,     // 3X3 Matrix
    HI_MATRIX_3_PLUS_4,     // 3X4 Matrix
} hi_matrix_type;

typedef struct {
    hi_matrix_type matrix_type; // 2X3 or 3X3 3X4 matrix(3D)
    hi_float matrix[3][4];
} hi_transform_matrix;
...
typedef enum {
    HI_FLIP_HOR = 0,
    HI_FLIP_VER = 1,
    HI_FLIP_BOTH = 2 // 同时使能xflip, yflip，相当于180度旋转
} hi_flip_mode;
...
/*
 * @brief set optional channel parameter attributes
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] opt_attr: optional channel parameter attributes
 * @return success: return 0
 *         fail: return error number
 */
 ...
 * @param [in] source_pic: pointer of vpc source picture info
 * @param [in] dest_pic: pointer of vpc dest picture info
 * @param [in] conf: pointer of csc config, support user to configure alpha channel
 * @param [in] milli_sec: -1 is block, 0 is no block, other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
...
/*
 * @brief : set chn workspace
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] workspace_param: pointer of workspace parameter
 * @param [in] param_cnt: parameter count
 * @return success: return 0
 *         fail: return error number
 */
 ...
 * @param [in] point_pair_info: point pair information
 * @param [out] matrix: affine transform matrix
 * @return success: return 0
 *         fail: return error number
 */
...
/*
 * @brief : get affine rotation matrix
 * @param [in] center_point: center of the rotation in the source image
 * @param [in] angle: rotation angle in degrees, and positive values mean counter-clockwise rotation
 * @param [in] scale: scale factor
 * @param [out] matrix: affine rotation matrix
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_get_rotation_matrix(hi_float_point center_point, hi_double angle, hi_double scale,
    hi_transform_matrix *matrix);

/*
 * @brief : affine by matrix
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] matrix: affine rotation matrix or transform matrix
 * @param [in] transform_param: warp transform parameter
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_warp_affine(hi_vpc_chn chn, hi_transform_matrix *matrix, hi_warp_transform_param *transform_param,
    hi_u32 *task_id, hi_s32 milli_sec);

/*
 * @brief : get chn affine lut
 * @param [in] point_pair_info: point pair information
 * @param [in] interpolation: remap mode, support bilinear
 * @param [out] remap_lut: remap lut information
 * @return success: return 0
 *         fail: return error number
 */
 ...
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_get_perspective_lut(hi_point_pair_info *point_pair_info, hi_u32 interpolation,
    hi_remap_lut *remap_lut);

/*
 * @brief : get chn remap lut
 * @param [in] map_param: map parameter
 * @param [out] remap_lut: remap lut information
 * @return success: return 0
 *         fail: return error number
 */
 ...
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] transform_param: warp transform parameter
 * @param [in] remap_lut: remap lut information
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
 */
hi_s32 hi_mpi_vpc_lut_remap(hi_vpc_chn chn, hi_warp_transform_param *transform_param, hi_remap_lut *remap_lut,
    hi_u32 *task_id, hi_s32 milli_sec);

/*
 * @brief : flip image
 * @param [in] chn: vpc channel id [0, VPC_MAX_CHN_NUM)
 * @param [in] flip_param: flip parameter
 * @param [in] milli_sec: -1 is block,0 is no block,other positive number is timeout
 * @param [out] task_id: pointer of the task ID, used to distinguish tasks.
 * @return success: return 0
 *         fail: return error number
